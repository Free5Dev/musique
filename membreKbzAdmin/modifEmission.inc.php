<?php 
    ...
    if(isset($_POST['btnModif'])){
        ...
        if(!empty($_POST['titre']) and !empty($_POST['invite']) and  !empty($_POST['url']) and !empty($_POST['photoKbzTv']) and !empty($_POST['realisateur']) and !empty($_POST['presentatrice']) and !empty($_POST['description'])){
           ...
           $reqUpdate=$bdd->prepare("UPDATE kbztv SET titre=?,invite=?,url=?,photoKbzTv=?,realisateur=?,presentatrice=?,description=? WHERE id=?");
           $reqUpdate->execute(array($_POST['titre'],$_POST['invite'],$_POST['url'],$_POST['photoKbzTv'],$_POST['realisateur'],$_POST['presentatrice'],$_POST['description'],$_POST['id']));
...
<body>
   ...
   <section class="container-fluid indexMusicFluid">
       <h1 >Modification de l'emission</h1>
