<?php 
    ...
    if(isset($_GET['btnSearch'])){
        ...
        if(!empty($_GET['search'])){
            $reqMusic=$bdd->prepare("SELECT * FROM musiques WHERE nomArtiste like ? ORDER BY id DESC LIMIT 20");
            $reqMusic->execute(array("%$_GET[search]%"));
        }
    }
...
<body>
   ...
   <section class="container-fluid indexMusicFluid">
       <h1 >Gestions des musiques</h1>
       <div class="row indexMusicRow">
           <div class="col-md-6 indexMusicCol">
               <p><a href="ajoutMusiques.inc.php" class="btn btn-primary">Ajouter une nouvelle music</a></p>
           </div>
           <div class="col-md-6 indexMusicCol">
       ...
       <div class="row indexMusicRow">
           <div class="col-md-12">
               <table class="table table-hover ">
                   ...
                   <?php while($donneesMusic=$reqMusic->fetch()) { ?>
                   <tr>
                       <td><?php echo htmlspecialchars($donneesMusic['nomArtiste']); ?></td>
                       <td><?php echo htmlspecialchars($donneesMusic['titre']); ?></td>
                       <td> <img src="<?php echo htmlspecialchars($donneesMusic['photo']); ?>" alt="" style="width:80px;height:50px;" class="img-responsive"> </td>
                       <td> <a href="modifMusiques.inc.php?ref=<?php echo $donneesMusic['id']; ?>">Modification</a> </td>
                       <td> <a href="supprimMusiques.inc.php?ref=<?php echo $donneesMusic['id']; ?>">Suppression</a> </td>
                   </tr>
                   <?php } $reqMusic->closeCursor(); ?>
               </table>
           </div>
       </div>
   </section>
