<?php 
    ...
    if(isset($_POST['btnModif'])){
        ...
        if(!empty($_POST['nomArtiste']) and !empty($_POST['titre']) and  !empty($_POST['url']) and !empty($_POST['description']) and !empty($_POST['photo'])){
           ...
           $reqUpdate=$bdd->prepare("UPDATE  musiques SET nomArtiste=?,titre=?,url=?,description=?,photo=?,idMusiqueCategorie=? WHERE id=?");
           $reqUpdate->execute(array($_POST['nomArtiste'],$_POST['titre'],$_POST['url'],$_POST['description'],$_POST['photo'],$_POST['categorie'],$_POST['id']));
